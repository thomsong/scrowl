        ...
        if (immediately) {
          if (_controller.enabled() && _enabled) {
            ...
            if (_options.duration > 0) {
              newProgress =
                (scrollPos - _scrollOffset.start) / (_scrollOffset.end - _scrollOffset.start);
            } else {
    ...
    var updateRelativePinSpacer = function () {
      if (
        ...
        (((_pinOptions.relSize.width || _pinOptions.relSize.autoFullWidth) &&
          _util.get.width(window) != _util.get.width(_pinOptions.spacer.parentNode)) ||
          (_pinOptions.relSize.height &&
            _util.get.height(window) != _util.get.height(_pinOptions.spacer.parentNode)))
      ) {
        updatePinDimensions();
      }
    };
      ...
      if (_controller && _pin && _state === SCENE_STATE_DURING && !_controller.info("isDocument")) {
        ...
        _controller._setScrollPos(
          _controller.info("scrollPos") -
            ((e.wheelDelta || e[_controller.info("vertical") ? "wheelDeltaY" : "wheelDeltaX"]) /
              3 || -e.detail * 30)
        );
      }
    };
    ...
    this.setPin = function (element, settings) {
      ...
      _pinOptions = {
        ...
        relSize: {
          ...
          autoFullWidth:
            sizeCSS.width === "auto" && inFlow && _util.isMarginCollapseType(pinCSS.display),
        },
