...
function ViewCourses(props: any) {
  ...
  const [courseViewCount, setCourseViewCount] = useState(5);
  const [recentData, setRecentData] = useState([]);

  const createNewCourse = async (blueprint) => {
    const result = await (window as any).ScrowlApp.course.newCourse(blueprint);
    if (!result) {
  ...
  const loadCourse = async (courseId) => {
    const loadCourseResult: any = await dispatch(courseActions.loadCourse(courseId));
    const courseData = loadCourseResult.payload;

    if (courseData.course && courseData.course.id === courseId) {
      dispatch(uiActions.setUIMode(UI_MODE.Designer));
    } else {
      console.error("Error loading course!", loadCourseResult);

      await dispatch(
        uiActions.showDialog({
          message: "Error Loading Course",
          detail: loadCourseResult.payload.error,
          type: "error",
        })
      );
    }
  ...
  useEffect(() => {
    const loadRecent = async () => {
      const result = await (window as any).ScrowlApp.course.loadRecent();
  ...
  const visibleCourses = [...recentData].splice(0, courseViewCount);

  return (
    <div className="scrowl scrowl--start" role="main">
      <div className="scrowl__start">
        <div className="scrowl__start__header">
          <h1>
            <div className="owl-logo owl-logo--start">
              <svg viewBox="0 0 125 125" xmlns="http://www.w3.org/2000/svg" width={50} height={50}>
                <path
                ...
                <circle cx="62.5" cy="69.22" fill="#fff" className="body" r={36} />
            ...
            Scrowl <small>v2</small>
          </h1>
        </div>
        <div className="scrowl__start__section">
          <h2>Start</h2>
          <ul className="nav flex-column">
            <li className="nav-item">
              ...
              >
                <span className="material-symbols-outlined me-2">library_add</span>New Course
              </a>
            </li>
          </ul>
        </div>
        <div className="scrowl__start__section">
          ...
          <ul className="nav flex-column">
            <li className="nav-item">
              ...
              >
                <span className="material-symbols-sharp me-2">assistant_direction</span>Beginner
                Tutorial Course
              </a>
            </li>
          </ul>
              ...
              {visibleCourses.map((course: any) => {
                return (
                  <li key={course.id} className="nav-item">
                    ...
                    >
                      <span className="material-symbols-outlined me-2">pages</span>
                      {course.name}{" "}
                      <small style={{ fontSize: "11px", color: "#666" }}>v{course.version}</small>
                    </a>
                  </li>
                );
              })}

              {courseViewCount < recentData.length ? (
                <li className="nav-item view-more">
                  <a
                    href="#"
                    onClick={(e) => {
                      e.preventDefault();
                      setCourseViewCount(courseViewCount + 5);
                    }}
                  >
                    View more courses...
                  </a>
