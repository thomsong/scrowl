...
function Fieldset(_props: PropsType) {
  const props: any = { ...defaultProps, ..._props };

  const contentPanelFocusField = useAppSelector((state) => state["course"].contentPanelFocusField);
  ...
  const validationErrors =
    selectedSlide && selectedSlide.validationErrors ? { ...selectedSlide.validationErrors } : {};
  ...
  Object.keys(props.fields).forEach((name) => {
    ...
    lastFieldInput = (
      <InputField
        ...
        onFocus={async (value) => {
          return await props.onFocus(fieldKey, value, { focusRange: schemaProps.focusRange });
        }}
        onBlur={async (value) => {
